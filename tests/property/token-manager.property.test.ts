 ...
 * 执行操作序列
 */
function executeOperations(manager: ExtendedTokenManager, operations: TokenOperation[]): void {
...
describe('Property P2: Token 内存存储不泄露', () => {
  ...
  // Token 生成器
  const tokenArb = fc
    .integer({ min: 1, max: 3600000 })
    .map(expiresInMs => generateJwtWithExpiration(expiresInMs))
  ...
  it('should store token only in memory after setAccessToken', () => {
    fc.assert(
      fc.property(tokenArb, token => {
        ...
        return (
          memoryToken === token && localStorageToken === null && localStorageTokenCamel === null
        )
      }),
      { numRuns: 100 }
    )
  ...
  it('should clear token from memory after clearAccessToken', () => {
    fc.assert(
      fc.property(tokenArb, token => {
        ...
        return memoryToken === null && localStorageToken === null && localStorageTokenCamel === null
      }),
      { numRuns: 100 }
    )
  })
  ...
  it('should handle login -> logout -> login sequence without localStorage leakage', () => {
    fc.assert(
      fc.property(tokenArb, tokenArb, (token1, token2) => {
        ...
        return afterLogin1 === null && afterLogout === null && afterLogin2 === null
      }),
      { numRuns: 100 }
    )
  })
  ...
  it('should handle rapid token updates without localStorage leakage', () => {
    fc.assert(
      fc.property(fc.array(tokenArb, { minLength: 5, maxLength: 50 }), tokens => {
        // 重置状态
        tokenManager._reset()
        localStorageData = {}
        ...
        return (
          localStorageToken === null && localStorageTokenCamel === null && memoryToken === lastToken
        )
      }),
      { numRuns: 50 }
    )
...
describe('Token Expiration Detection', () => {
  ...
  it('should correctly report hasValidToken', () => {
    fc.assert(
      fc.property(fc.integer({ min: 60000, max: 3600000 }), expiresInMs => {
        tokenManager._reset()
...
describe('Token Expiration Time', () => {
  ...
  it('should correctly extract expiration time from token', () => {
    fc.assert(
      fc.property(fc.integer({ min: 1000, max: 3600000 }), expiresInMs => {
        const now = Date.now()
        const token = generateJwtWithExpiration(expiresInMs)
