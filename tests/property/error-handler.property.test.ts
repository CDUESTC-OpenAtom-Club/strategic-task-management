 ...
 * 生成网络错误（无响应）
 */
const networkErrorArb = fc
  .tuple(axiosConfigArb, fc.constantFrom('ERR_NETWORK', 'ECONNREFUSED', 'ETIMEDOUT'))
  .map(([config, code]) => {
...
describe('Error Handler Property Tests', () => {
  ...
  describe('边界情况', () => {
    ...
    it('处理非标准响应格式', () => {
      fc.assert(
        fc.property(
          fc.oneof(fc.string(), fc.integer(), fc.array(fc.string()), fc.constant(null)),
          data => {
            const error = new Error('Request failed') as AxiosError
            error.isAxiosError = true
            error.config = {
