...
import { getProgressColor, getProgressStatus, getStatusTagType } from '@/shared/lib/utils'
...
describe('Property 5: Progress Color Rule Consistency', () => {
  ...
  it('should return exception status for progress < 50', () => {
    fc.assert(
      fc.property(fc.integer({ min: 0, max: 49 }), progress => {
        const status = getProgressStatus(progress)
        const color = getProgressColor(progress)
...
describe('Property 4: Status Color Mapping Consistency', () => {
  ...
  it('should map danger statuses to danger tag type', () => {
    fc.assert(
      fc.property(fc.constantFrom(...dangerStatuses), status => {
  ...
  it('should map info statuses to info tag type', () => {
    fc.assert(
      fc.property(fc.constantFrom(...infoStatuses), status => {
  ...
  it('should map primary statuses to primary tag type', () => {
    fc.assert(
      fc.property(fc.constantFrom(...primaryStatuses), status => {
