...
| 条目  | 状态 | 最终验收证据                                           |
| ----- | ---- | ------------------------------------------------------ |
| P0-01 | ✅   | `npm run arch:check` 稳定输出基线，含阈值检测          |
| P0-02 | ✅   | `@ts-nocheck` 文件数 = 0，新增会被 arch:check 阻断     |
| P0-03 | ✅   | ESLint `no-explicit-any`、`ban-ts-comment` 已升至 warn |

P0 阶段关键成果：

- `@ts-nocheck` 从 105 → 0（清零）
- 最大 Vue 文件从 8357 行 → 1624 行（降幅 80.5%）
- 顶层旧入口文件数从 23 → 0（归零）
- 权限 hook 实现从 4 → 1（唯一化）
- 测试通过率从基线未建立 → 99.8%（1242/1245，0 failures）
- ESLint 类型规则从关闭 → warn（P0-03 phase 1）

P0 期间共产生 33 个原子提交，按功能分为：

- 基础设施与闸门（3 commits）
- 大文件拆分与兼容层清理（8 commits）
- `@ts-nocheck` 清零（14 commits）
- 测试修复与规则收紧（8 commits）
