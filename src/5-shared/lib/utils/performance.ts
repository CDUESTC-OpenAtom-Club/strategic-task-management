...
function getApiLatencyStats(): PerformanceMetrics['apiLatency'] {
  ...
  for (const record of apiLatencyRecords) {
    ...
    const urlDurations = byUrl.get(key)
    if (urlDurations) {
      urlDurations.push(record.duration)
    }
  ...
  const averageMs =
    totalRequests > 0 ? allDurations.reduce((sum, d) => sum + d, 0) / totalRequests : 0
...
export function printPerformanceReport(metrics?: PerformanceMetrics): void {
  ...
  console.log(
    `Page Load Time: ${data.pageLoadTime !== null ? `${data.pageLoadTime.toFixed(2)} ms` : 'N/A'}`
  )
  console.log(
    `Resource Load Time: ${data.resourceLoadTime !== null ? `${data.resourceLoadTime.toFixed(2)} ms` : 'N/A'}`
  )
  console.groupEnd()
