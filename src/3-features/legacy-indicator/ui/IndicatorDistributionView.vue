...
import indicatorApi from '@/5-shared/api/indicator'
...
const selectParentIndicator = (indicator: StrategicIndicator) => {
  ...
  // 继承父指标的类型（用户仍可修改）
  newIndicatorForm.value.type1 = indicator.type1 || (indicator.isQualitative ? '定性' : '定量')
  // 如果是定量指标，自动生成12个月里程碑
  if (newIndicatorForm.value.type1 === '定量') {
...
const _handleBatchApprove = (college: string) => {
  ...
  const pendingIndicators = childIndicators.filter(
    i => getChildStatus(i as StrategicIndicator) === 'pending'
  )
