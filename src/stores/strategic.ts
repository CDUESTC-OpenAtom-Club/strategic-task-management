...
import {
  ...
  hasBackendUpdates as hasBackendUpdatesUtil
} from '@/utils/dataMappers/indicatorMapper'
...
export const useStrategicStore = defineStore('strategic', () => {
  ...
  const loadIndicatorsFromApi = async (
    ...
    try {
      logger.info(
        `[Strategic Store] Loading indicators for year ${year} from API... (attempt ${retryCount + 1}/${maxRetries + 1})`
      )
      const response = await strategicApi.getIndicatorsByYear(year)
      if (response.success && response.data) {
        const converted = response.data.map(vo =>
          strategicApi.convertIndicatorVOToStrategicIndicator(vo)
        )
      ...
      if (isTimeout && retryCount < maxRetries) {
        logger.warn(
          `[Strategic Store] Request timeout, retrying... (${retryCount + 1}/${maxRetries})`
        )
        // 等待一段时间后重试（指数退避）
  ...
  const loadTasksFromApi = async (year: number, retryCount = 0): Promise<StrategicTask[]> => {
    ...
    try {
      logger.info(
        `[Strategic Store] Loading tasks for year ${year} from API... (attempt ${retryCount + 1}/${maxRetries + 1})`
      )
  ...
  const loadIndicatorsByYear = async (year: number) => {
    ...
    if (useApiData.value) {
      try {
        ...
        logger.info(
          `[Strategic Store] Successfully loaded ${apiIndicators.length} indicators and ${apiTasks.length} tasks from API`
        )
  ...
  const addIndicator = async (indicator: StrategicIndicator) => {
    ...
    try {
      ...
      logger.info(
        `[Strategic Store] indicator.taskContent = "${indicator.taskContent}", type: ${typeof indicator.taskContent}`
      )

      if (indicator.taskContent) {
        logger.info(
          `[Strategic Store] Looking for taskId for taskContent: ${indicator.taskContent}`
        )
        ...
        const existingIndicator = indicators.value.find(
          i => i.taskContent === indicator.taskContent
        )
        if (existingIndicator && existingIndicator.taskId) {
          taskId = Number(existingIndicator.taskId)
          logger.info(
            `[Strategic Store] Found taskId ${taskId} from existing indicator (id: ${existingIndicator.id}) for taskContent: ${indicator.taskContent}`
          )
        } else {
          logger.warn(
            `[Strategic Store] No existing indicator found with taskContent: ${indicator.taskContent}`
          )

          // 如果现有指标中找不到，再从 activeTasks 中查找
          logger.info(
            `[Strategic Store] Searching in activeTasks (${activeTasks.value.length} tasks)`
          )
          ...
          if (matchingTask) {
            ...
            logger.info(
              `[Strategic Store] Found task with id ${taskId} for taskContent: ${indicator.taskContent}`
            )
          } else {
            // 如果都找不到，使用默认 taskId = 1
            logger.warn(
              `[Strategic Store] No matching task found for taskContent: ${indicator.taskContent}, using default taskId: 1`
            )
          }
        }
      } else {
      ...
      logger.info(
        `[Strategic Store] Creating indicator with taskId: ${taskId}, taskContent: ${indicator.taskContent}`
      )
      ...
      const request = {
        ...
        // 里程碑数据
        milestones:
          indicator.milestones?.map((ms, index) => ({
      ...
      if (response.success && response.data) {
        ...
        if (indicator.milestones && indicator.milestones.length > 0) {
          try {
            ...
            logger.info(
              `[Strategic Store] Created ${indicator.milestones.length} milestones for indicator ${createdIndicatorId}`
            )
          } catch (milestoneErr) {
        ...
        if (index !== -1) {
          ...
          const convertedIndicator = strategicApi.convertIndicatorVOToStrategicIndicator(
            response.data
          )
          ...
          if (indicator.milestones && indicator.milestones.length > 0) {
            ...
            if (reloadResponse.success && reloadResponse.data) {
              indicators.value[index] = strategicApi.convertIndicatorVOToStrategicIndicator(
                reloadResponse.data
              )
            } else {
  ...
  const updateIndicator = async (id: string, updates: Partial<StrategicIndicator>) => {
    ...
    // 检查是否有需要同步到后端的字段
    const shouldSyncToBackend = hasBackendUpdatesUtil(updates)
    logger.debug(`[Strategic Store] Checking if updates need backend sync:`, {
      indicatorId: id,
      updates,
      shouldSyncToBackend
    })
    ...
    if (shouldSyncToBackend) {
      try {
        ...
        logger.debug(
          `[Strategic Store] Converted update request for indicator ${id}:`,
          updateRequest
        )

        await indicatorApi.updateIndicator(id, updateRequest)
        logger.info(
          `[Strategic Store] Successfully synced indicator ${id} to backend`,
          updateRequest
        )
      ...
      logger.debug(
        `[Strategic Store] No backend sync needed for indicator ${id}, updates are local-only`
      )
    }
  }
  ...
  const validateCurrentData = (): ValidationResult => {
    ...
    // 合并所有验证结果
    const mergedResult =
      indicatorResults.length > 0 ? mergeResults(...indicatorResults) : createEmptyResult()
   ...
   */
  const replaceIndicatorId = (
    tempId: string,
    realId: string,
    extraUpdates?: Partial<StrategicIndicator>
  ) => {
    const index = indicators.value.findIndex(i => i.id === tempId)
    if (index !== -1) {
