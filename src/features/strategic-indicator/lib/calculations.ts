 ...
 */
export function calculateWeightedCompletionRate(
...
export function validateWeightSum(
  ...
  const totalWeight = indicators.reduce(
    (sum, indicator) => sum + (indicator.weight || 0),
    0
  )
...
export function calculateAggregateStatistics(
  ...
  const completedCount = indicators.filter(
    i => (i.completionRate || 0) >= 100
  ).length
  
  const totalCompletion = indicators.reduce(
    (sum, i) => sum + (i.completionRate || 0),
    0
  )
