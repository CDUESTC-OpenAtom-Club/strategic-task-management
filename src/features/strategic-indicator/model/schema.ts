...
export const indicatorCreateSchema = z.object({
  name: z.string()
    .min(1, '指标名称不能为空')
    .max(VALIDATION_RULES.NAME_MAX_LENGTH, `指标名称不能超过${VALIDATION_RULES.NAME_MAX_LENGTH}个字符`),
  
  code: z.string()
    .max(VALIDATION_RULES.CODE_MAX_LENGTH, `指标编码不能超过${VALIDATION_RULES.CODE_MAX_LENGTH}个字符`)
    .optional(),
  
  description: z.string()
    .max(VALIDATION_RULES.DESCRIPTION_MAX_LENGTH, `描述不能超过${VALIDATION_RULES.DESCRIPTION_MAX_LENGTH}个字符`)
    .optional(),
...
export const distributeRequestSchema = z.object({
  targetOrgIds: z.array(z.number().positive())
    .min(1, '请至少选择一个目标组织'),
